...
- 启动后端：`cd backend-python && .venv/Scripts/python.exe -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000`
- 启动前端：`cd frontend-vue && npm run dev`（:5173，/api 代理 8000）
- 红线：环境+启动完成后不改业务代码，等用户“开始修复”指令

## 工作约定（WMS 任务执行流程）
- 实现按 plan.md 逐项推进；每改完一项，必须向用户**附带该项的测试方法/验证步骤**（命令行 curl / 前端操作步骤 / 预期结果），等用户确认「继续」再下一项。
...
- 必做2：库存查询（后端 inventory.py 待③；前端 InventoryView.vue 待④）
