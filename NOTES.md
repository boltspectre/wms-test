...
## 十、UI 外观优化（已落地：可一键切换「原版 / 美化」双主题）

> 设计原则（按面试官诉求）：**默认 = 原版主题**（即 Element Plus 默认外观，面试官最熟悉、可直接用于功能测试）；提供**醒目的主题切换开关**一键切到**美化主题**（侧边栏 + 仪表盘 + 卡片化 + 品牌化），用于观赏。两种主题**复用完全相同的视图组件与 API 调用**，因此「美化主题」同样具备**完整的后端交互**（入库/库存/出库/删除全部可用），绝不只是换皮。所有改动**仅限前端样式与布局**，后端 `API_SPEC` 的 camelCase 契约、接口路径、错误包络 `{code,message,data}` 一律不变。
...
按"双主题、默认原版"的诉求，已把 P0/P1 的大部分做成**可一键切换**的实装，而非停留在规划：

| 落地项 | 对应路线 | 实现方式 |
|--------|----------|----------|
| **主题切换开关（醒目）** | — | 顶栏右侧常驻胶囊式开关「原版主题 / 美化主题」，带图标；选择写入 `localStorage`，刷新保持；默认 `original` |
| **原版主题 = 熟悉外观** | — | 不注入任何美化 Token，回退 Element Plus 默认；保持原顶部蓝色水平菜单 + 朴素页面，面试官可直接按原样测试 |
| **美化主题：设计 Token** | A | `styles/variables.css` 全部 Token（主色/语义色/阴影/圆角）**仅作用于 `[data-theme='beautified']`**，不污染原版 |
| **美化主题：侧边栏 + 顶栏品牌** | B | `App.vue`：美化态渲染深色渐变可折叠侧边栏（含图标菜单 + 仪表盘入口）+ 白色品牌顶栏 |
| **美化主题：卡片化容器** | B | 视图外层 `.wb-view-wrap` 在美化态变为白卡 + 圆角 + 阴影；原版态透明无侵入 |
| **仪表盘首页（Dashboard）** | B | 新增 `DashboardView.vue`：4 张统计卡（商品种类/库存总量/低库存/出库单）+ 低库存进度条列表 + 近 7 日出入库 SVG 柱状趋势；数据来自既有 `getInventory/getProducts/getOutboundOrders/getInboundOrders` |
| **状态视觉（Tag/进度/语义色）** | C/E | 低库存用进度条 + 红/橙渐变；统计卡图标渐变底；表格表头轻着色 |
| **路由切换淡入** | G | 美化态 `<transition name="wb-fade">` 包裹 `keep-alive` 视图 |

**验证结果**：`npm run build`（vue-tsc 类型检查 0 错误 + Vite 打包 1664 模块全部成功，产出含 `DashboardView` chunk）通过；新增/修改文件：`src/App.vue`、`src/views/DashboardView.vue`、`src/styles/variables.css`、`src/styles/global.css`、`src/api/index.ts`（补 `getInboundOrders`）、`src/router/index.ts`（补 `/dashboard`）、`src/main.ts`（引入样式）。

> 约束：以上优化**只动前端样式与布局**，后端 `API_SPEC` 的 camelCase 契约、接口路径、错误包络 `{code,message,data}` 一律不变，避免影响已通过的验收。`dist/` 已被 `.gitignore` 忽略，构建产物不入库。
